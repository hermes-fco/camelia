...
# Listens on worker.model.deepseek.completion with queue group for auto-scale.
# Uses Proc::Async for non-blocking HTTP calls to DeepSeek API.
...
note "🟡 Worker-Model-DeepSeek connecting NATS ($nats-url)...";
...
# Subscribe with queue group for auto-scale — NATS distributes requests
# across all instances in the group
my $sub = $nats.subscribe: 'worker.model.deepseek.completion',
    :queue('worker-model-deepseek');
note "🟢 Subscribed worker.model.deepseek.completion (queue: worker-model-deepseek), SID={$sub.sid}. Entering react...";

# Health check (unified worker pattern)
my $health-sub = $nats.subscribe: 'health.check.worker.model.deepseek';
...
react {
    ...
    whenever $health-sub.supply -> $msg {
        my $rt = $msg.?reply-to;
        if $rt {
            $nats.publish: $rt, to-json({ :status<ok>, :service<worker-model-deepseek> });
        }
    }
}
